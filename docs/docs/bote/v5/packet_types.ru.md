...
Информация о поддерживаемых алгоритмах шифрования и подписи находятся в разделе [Криптография](cryptography.md)

## 1. Пакеты данных

**Все** `Пакеты данных` начинаются с 1 байта для `Типа пакета` и следующего за ним 1 байта для `Версии протокола`.
...
### 1.4 Пакет информации для удаления
